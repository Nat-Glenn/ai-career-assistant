...
import { ResumeTailorResults } from "./resume-tailor-results";
...
export function ResumeTailorForm() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        setError(json.error?.message ?? "Tailoring failed. Please try again.");
        return;
      }
    ...
    <div className="space-y-8">
      ...
      {isLoading && (
        <p className="rounded-xl border border-card-border bg-card px-4 py-3 text-sm text-muted">
          Tailoring your resume with AI — this may take a few seconds…
        </p>
      )}
      ...
      {result && <ResumeTailorResults result={result} />}
    </div>
  );
}
